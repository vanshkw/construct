...
export default function Dashboard() {
  return (
    <div className="min-h-screen pt-20 pb-12 px-4 sm:px-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div 
            ...
            className="lg:col-span-2 glass-card p-8 relative overflow-hidden flex flex-col justify-center group"
        >
            ...
            <div className="relative z-10">
                ...
                <h2 className="text-3xl font-bold text-white mb-2">Good Evening, Vansh!</h2>
                <p className="text-slate-300 mb-8 max-w-lg leading-relaxed">
                    Your consistency is paying off. Based on your recent graph problems, we've curated a specific challenge for you.
                </p>
                ...
                <div className="flex gap-4">
                    ...
                    <button className="px-6 py-2.5 rounded-lg border border-white/10 hover:bg-white/5 text-white font-medium text-sm transition-colors">
                        View Analysis
                    </button>
                </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          {/* Search & Filters (Takes up 3 columns) */}
          <div className="lg:col-span-3 space-y-6">
              ...
              >
                  <div className="relative flex-1 group">
                      ...
                      <input 
                          ...
                          className="w-full bg-[#0F1629] border border-white/10 text-white rounded-xl pl-12 pr-4 py-3.5 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/50 transition-all placeholder:text-slate-600 shadow-inner"
                      />
                  </div>
                  <button className="px-5 py-3.5 bg-[#0F1629] border border-white/10 rounded-xl text-slate-400 hover:text-white hover:border-violet-500/50 transition-all flex items-center gap-2 shadow-sm">
              ...
              <div className="space-y-3">
                  <div className="flex items-center justify-between px-2 mb-2">
                      <h3 className="text-white font-semibold">Recent Problems</h3>
          ...
          {/* Skill Rings (Side Column) */}
          <motion.div 
             initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
             className="glass-card p-6 flex flex-col h-full"
          >
              <h3 className="text-white font-semibold mb-1">Skill Mastery</h3>
              <p className="text-xs text-slate-500 mb-6">Based on solved difficulty</p>
              ...
              <div className="grid grid-cols-2 gap-y-8 gap-x-4 flex-1 content-center">
                  {SKILLS.map((cat, index) => (
                      <div key={index} className="flex flex-col items-center group cursor-pointer">
                          ...
                          <span className="text-xs text-slate-400 text-center font-medium group-hover:text-white transition-colors">{cat.name}</span>
                      </div>
                  ))}
              </div>
          </motion.div>
    ...
    return (
        ...
        >
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h4 className="text-white font-medium text-sm group-hover:text-violet-400 transition-colors">{problem.title}</h4>
            ...
            <div className="hidden md:flex items-center gap-6">
                <div className="flex gap-2">
                    {problem.tags.map((tag, i) => (
                        <span key={i} className="px-2 py-1 rounded text-[10px] font-medium bg-white/5 text-slate-400 border border-white/5">
                            {tag}
                        </span>
                    ))}
                </div>
