    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 w-full z-50 transition-all duration-300 border-b ${
          isScrolled
            ? "bg-[#0B1120]/80 backdrop-blur-xl border-white/10 shadow-lg shadow-violet-500/5"
            ...
            <div className="flex items-center gap-4">
              ...
              {isLoggedIn ? (
                ...
                <>
                  ...
                  <Link href="/login" className="px-5 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-sm font-medium text-white transition-all flex items-center gap-2 group">
                    <LogIn size={16} className="text-violet-400 group-hover:text-white transition-colors" />
                    Sign In
                  </Link>
          ...
          >
            <div className="p-4 flex flex-col gap-2">
              {navItems.map((item) => (
                ...
                >
                  <item.icon size={18} className={pathname === item.href ? "text-violet-400" : ""} />
                  {item.name}
                </Link>
              ))}
              ...
              {!isLoggedIn && (
                 <Link
                   href="/login"
                   onClick={() => setIsMobileMenuOpen(false)}
                   className="mt-2 flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white text-sm font-medium shadow-lg shadow-violet-500/20"
                 >
                   <LogIn size={18} />
                   Sign In
                 </Link>
